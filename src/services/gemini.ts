...
export class GeminiService {
	...
	async translateLongTextAndStream(
		...
		return openRouterService.translateLongText(env, userId, text, sourceLang, targetLang, sourceLangName, targetLangName, ctx);
	}
	...
	async classifyText(
		...
		// 2. Cache Miss: Perform Classification using OpenRouter
		const { OpenRouterService } = await import('./openrouter');
		const openRouterService = new OpenRouterService();

		const result = await openRouterService.classifyText(env, userId, text, ctx);
		const classificationType = result.type;
	...
	async translateWordAndStream(
		...
		return openRouterService.translateWord(env, userId, text, sourceLang, targetLang, sourceLangName, targetLangName, ctx);
	}
	...
	async translateImageAndStream(
		...
		return openRouterService.translateImage(
			env,
			userId,
			imageBase64,
			mimeType,
